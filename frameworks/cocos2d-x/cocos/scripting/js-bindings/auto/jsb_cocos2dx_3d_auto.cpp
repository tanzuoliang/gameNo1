...
bool js_cocos2dx_3d_Animate3D_registerOnFrameChange(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    bool ok = true;
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    cocos2d::Animate3D* cobj = (cocos2d::Animate3D *)(proxy ? proxy->ptr : NULL);
    JSB_PRECONDITION2( cobj, cx, false, "js_cocos2dx_3d_Animate3D_registerOnFrameChange : Invalid Native Object");
   // if (argc == 1) {
...
//        ok &= jsval_to_int32(cx, args.get(1), (int32_t *)&arg1);
        do {
            if(JS_TypeOfValue(cx, args.get(0)) == JSTYPE_FUNCTION)
            {
                ...
                auto lambda = [=](int frameIndex,int totalIndex) -> void {
                    JSB_AUTOCOMPARTMENT_WITH_GLOBAL_OBJCET
                    jsval largv[2];
                    if (frameIndex) {
                        largv[0] = INT_TO_JSVAL(frameIndex);
                    } else {
                        largv[0] = JSVAL_NULL;
                    };
                    
                    if (totalIndex) {
                        largv[1] = INT_TO_JSVAL(totalIndex);
                    } else {
                        largv[1] = JSVAL_NULL;
                    };
            ...
            ;
        JSB_PRECONDITION2(ok, cx, false, "js_cocos2dx_3d_Animate3D_registerOnFrameChange : Error processing arguments");
        cobj->registerOnFrameChange(arg0);
        args.rval().set(JSVAL_VOID);
        return true;
   // }
    
    JS_ReportError(cx, "js_cocos2dx_3d_Animate3D_registerOnFrameChange : wrong number of arguments: %d, was expecting %d", argc, 2);
    return false;
}
...
void js_register_cocos2dx_3d_Animate3D(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec funcs[] = {
        ...
        JS_FN("registerOnFrameChange", js_cocos2dx_3d_Animate3D_registerOnFrameChange, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FS_END
    };
...
bool js_cocos2dx_3d_Sprite3D_useOutline(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    ...
    if (argc == 1) {
        std::string material;
        ok &= jsval_to_std_string(cx, args.get(0), &material);
        JSB_PRECONDITION2(ok, cx, false, "js_cocos2dx_3d_Sprite3D_genMaterial : Error processing arguments");
        cobj->useOutline(material);
        args.rval().setUndefined();
        return true;
    }
    else if(argc == 0){
    ...
    JS_ReportError(cx, "js_cocos2dx_3d_Sprite3D_useOutline : wrong number of arguments: %d, was expecting %d", argc, 0);
    return false;
}

bool js_cocos2dx_3d_Sprite3D_unuseOutline(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    ...
    JS_ReportError(cx, "js_cocos2dx_3d_Sprite3D_unuseOutline : wrong number of arguments: %d, was expecting %d", argc, 0);
    return false;
}
...
void js_register_cocos2dx_3d_Sprite3D(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec funcs[] = {
        ...
        JS_FN("useOutline", js_cocos2dx_3d_Sprite3D_useOutline, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FN("unuseOutline", js_cocos2dx_3d_Sprite3D_unuseOutline, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
